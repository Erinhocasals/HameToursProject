...
@section('content')
    <div class="row page-titles">
        <ol class="breadcrumb">
            <li class="breadcrumb-item active"><a href="javascript:void(0)">Showcases</a></li>
        </ol>
    </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    {{-- <form method="GET" action="{{ url('/admin/showcases') }}" accept-charset="UTF-8" class="float-right my-2 form-inline my-lg-0" role="search">
                    ...
                    <div class="table-responsive">
                        <table id="example3" class="display">
                            ...
                            @foreach($showcases as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->title }}</td>
                                    <td><a href="{{ $item->showcaseable->backendLink() }}"<span class="badge badge-sm light badge-primary">{{ $item->showcaseable->title }}</span></a></td>
                                    <td>{{ $item->enabled }}</td>
                                    <td class="text-center">
                                        <a href="{{ url('/admin/showcases/' . $item->id) }}" title="Detalles"><button class="shadow btn btn-info btn-xs sharp me-1"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                                        <a href="{{ url('/admin/showcases/' . $item->id . '/edit') }}" title="Editar"><button class="shadow btn btn-primary btn-xs sharp me-1"><i class="fa fa-pencil" aria-hidden="true"></i></button></a>

                                        <form method="POST" action="{{ url('/admin/showcases' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
                        ...
                        {{-- <div class="pagination-wrapper"> {!! $showcases->appends(['search' => Request::get('search')])->render() !!} </div> --}}
                    </div>
