...
@section('content')
    <div class="row page-titles">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><a href="javascript:void(0)">Textos</a></li>
        </ol>
    </div>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    {{-- <a href="{{ url('/admin/texts/create') }}" class="btn btn-primary btn-sm" title="Adicionar nuevo Texto">
                        <i class="fa fa-plus" aria-hidden="true"></i> Adicionar nuevo
                    </a> --}}
                </div>
                <div class="card-body">
                    {{-- <form method="GET" action="{{ url('/admin/texts') }}" accept-charset="UTF-8" class="float-right my-2 form-inline my-lg-0" role="search">
                    ...
                    <div class="table-responsive">
                        <table id="example3" class="display">
                            ...
                            @foreach($texts as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->title }}</td>
                                    <td>{{ $item->enabled }}</td>
                                    <td> @if ($item->textable->title) <a href="{{ $item->textable->backendLink() }}" <span class="badge badge-sm light badge-primary"> {{ $item->textable->title }}</span> </a> @endif </td>
                                    <td class="text-center">
                                        <a href="{{ url('/admin/texts/' . $item->id) }}" title="Detalles"><button class="shadow btn btn-info btn-xs sharp me-1"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                                        <a href="{{ url('/admin/texts/' . $item->id . '/edit') }}" title="Editar"><button class="shadow btn btn-primary btn-xs sharp me-1"><i class="fa fa-pencil" aria-hidden="true"></i></button></a>

                                        <form method="POST" action="{{ url('/admin/texts' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
                        ...
                        <div class="pagination-wrapper"> {!! $texts->appends(['search' => Request::get('search')])->render() !!} </div>
                    </div>
