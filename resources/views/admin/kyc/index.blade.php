        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    {{-- <a href="{{ url('/admin/kyc/create') }}" class="btn btn-primary btn-sm" title="Adicionar">
                        <i class="fa fa-plus" aria-hidden="true"></i> Adicionar nuevo
                    </a> --}}
                </div>
                <div class="card-body">
                    {{-- <form method="GET" action="{{ url('/admin/kyc') }}" accept-charset="UTF-8" class="float-right my-2 form-inline my-lg-0" role="search">
                    ...
                    <div class="table-responsive">
                            ...
                            @foreach($kyc as $item)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="{{ url('/admin/kyc/' . $item->id) }}" title="Detalles"><button class="shadow btn btn-info btn-xs sharp me-1"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                                        <a href="{{ url('/admin/kyc/' . $item->id . '/edit') }}" title="Cambiar aprobación"><button class="shadow btn btn-primary btn-xs sharp me-1"><i class="fa fa-check" aria-hidden="true"></i></button></a>

                                        <form method="POST" action="{{ url('/admin/kyc' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
                        ...
                        <div class="pagination-wrapper"> {!! $kyc->appends(['search' => Request::get('search')])->render() !!} </div>
                    </div>
