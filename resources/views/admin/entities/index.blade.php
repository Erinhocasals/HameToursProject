        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ url('/admin/entities/create') }}" class="btn btn-primary btn-sm" title="Adicionar nueva Entity">
                        <i class="fa fa-plus" aria-hidden="true"></i> Adicionar nuevo
                    </a>
                </div>
                <div class="card-body">
                    {{-- <form method="GET" action="{{ url('/admin/entities') }}" accept-charset="UTF-8" class="float-right my-2 form-inline my-lg-0" role="search">
                    ...
                    <div class="table-responsive">
                        <table id="example3" class="display">
                            ...
                            @foreach($entities as $item)
                                <tr>
                                    <td>{{ $item->id }}</td>
                                    <td>{{ $item->title }}</td>
                                    <td>@if ($item->enabled) Sí @else No @endif</td>
                                    <td><a href="{{ $item->category->backendLink() }}"<span class="badge badge-sm light badge-primary">{{ $item->category->title }}</span></a></td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        <a href="{{ url('/admin/entities/' . $item->id) }}" title="Detalles"><button class="shadow btn btn-info btn-xs sharp me-1"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                                        <a href="{{ url('/admin/entities/' . $item->id . '/edit') }}" title="Editar"><button class="shadow btn btn-primary btn-xs sharp me-1"><i class="fa fa-pencil" aria-hidden="true"></i></button></a>

                                        <form method="POST" action="{{ url('/admin/entities' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
                        ...
                        {{-- <div class="pagination-wrapper"> {!! $entities->appends(['search' => Request::get('search')])->render() !!} </div> --}}
                    </div>
