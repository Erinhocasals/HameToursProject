...
@section('content')
    <div class="row page-titles">
        <ol class="breadcrumb">
            <li class="breadcrumb-item active"><a href="javascript:void(0)">Configuración</a></li>
        </ol>
    </div>
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    @hasanyrole('super-admin')
                        <a href="{{ url('/admin/data/create') }}" class="btn btn-primary btn-sm" title="Adicionar nueva Data">
                            <i class="fa fa-plus" aria-hidden="true"></i> Adicionar nueva
                        </a>
                    @endhasanyrole
                </div>
                <div class="card-body">
                    {{-- <form method="GET" action="{{ url('/admin/data') }}" accept-charset="UTF-8" class="float-right my-2 form-inline my-lg-0" role="search">
                    ...
                    <div class="table-responsive">
                        <table id="example3" class="display">
                            <thead>
                                <tr>
                                    <th>#</th><th>Título</th><th>Key</th><th>Value</th><th class="text-center">Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                            @foreach($data as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->title }}</td><td>{{ $item->key }}</td><td>{{ $item->value }}</td>
                                    <td class="text-center">
                                        <a href="{{ url('/admin/data/' . $item->id) }}" title="Detalles"><button class="shadow btn btn-info btn-xs sharp me-1"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                                        <a href="{{ url('/admin/data/' . $item->id . '/edit') }}" title="Editar"><button class="shadow btn btn-primary btn-xs sharp me-1"><i class="fa fa-pencil" aria-hidden="true"></i></button></a>

                                        <form method="POST" action="{{ url('/admin/data' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
