    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <div class="card-body">
                    {{-- <form method="GET" action="{{ url('/admin/log') }}" accept-charset="UTF-8" class="float-right my-2 form-inline my-lg-0" role="search">
                    ...
                    <div class="table-responsive">
                        <table id="logs" class="display">
                            <thead>
                                <tr>
                                    <th>#</th><th>Título</th><th>Fecha</th><th>Elemento</th><th>Usuario</th><th class="text-center">Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                            @foreach($log as $item)
                                <tr>
                                    ...
                                    <td><a href="#" ><span class="badge badge-sm light badge-primary">{{ str_replace("App\\Models\\", "", $item->logable_type)  }}</span></a> </td>
                                    <td><a href="{{ route('admin.users.show', ['user' => $item->user->id]) }}" ><span class="badge badge-sm light badge-primary">{{ $item->user->name }}</span></a> </td>
                                    <td class="text-center">
                                        <a href="{{ url('/admin/log/' . $item->id) }}" title="Detalles"><button class="shadow btn btn-info btn-xs sharp me-1"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                                        {{-- <a href="{{ url('/admin/log/' . $item->id . '/edit') }}" title="Editar"><button class="shadow btn btn-primary btn-xs sharp me-1"><i class="fa fa-pencil" aria-hidden="true"></i></button></a>

                                        <form method="POST" action="{{ url('/admin/log' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
