...
@section('content')
    <div class="row page-titles">
        <ol class="breadcrumb">
            <li class="breadcrumb-item active"><a href="javascript:void(0)">Redes Sociales</a></li>
        </ol>
    </div>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ url('/admin/social/create') }}" class="btn btn-primary btn-sm" title="Adicionar nueva Social">
                        <i class="fa fa-plus" aria-hidden="true"></i> Adicionar nueva
                    </a>
                </div>
                <div class="card-body">
                    {{-- <form method="GET" action="{{ url('/admin/social') }}" accept-charset="UTF-8" class="float-right my-2 form-inline my-lg-0" role="search">
                    ...
                    <div class="table-responsive">
                        <table id="example3" class="display">
                            <thead>
                                <tr>
                                    <th>#</th><th>Título</th><th>Icono</th><th>Habilitado</th><th class="text-center">Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                            @foreach($socials as $item)
                                <tr>
                                    <td>{{ $item->id }}</td>
                                    <td>{{ $item->title }}</td>
                                    <td><img class="h-4" src="{{ asset('storage/' . $item->image->url ) }}"></td>
                                    <td>{{ $item->enabled }}</td>
                                    <td class="text-center">
                                        <a href="{{ url('/admin/social/' . $item->id) }}" title="Detalles"><button class="shadow btn btn-info btn-xs sharp me-1"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                                        <a href="{{ url('/admin/social/' . $item->id . '/edit') }}" title="Editar"><button class="shadow btn btn-primary btn-xs sharp me-1"><i class="fa fa-pencil" aria-hidden="true"></i></button></a>

                                        <form method="POST" action="{{ url('/admin/social' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
                        ...
                        <div class="pagination-wrapper"> {!! $socials->appends(['search' => Request::get('search')])->render() !!} </div>
                    </div>
