...
@section('content')
    <div class="row page-titles">
        <ol class="breadcrumb">
            <li class="breadcrumb-item active"><a href="javascript:void(0)">@if ($type == 1) Regiones @else Definiciones @endif</a></li>
        </ol>
    </div>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">@if ($type == 1) Regiones @else Definiciones @endif</h4>
                    <a href="{{ route('admin.categories.create', ['type' => $type]) }}" class="shadow btn btn-primary btn-sm" title="Adicionar nueva Category">
                        <i class="fa fa-plus" aria-hidden="true"></i> Adicionar nueva
                    </a>
                </div>
                <div class="card-body">
                    {{-- <form method="GET" action="{{ url('/admin/categories') }}" accept-charset="UTF-8" class="float-right my-2 form-inline my-lg-0" role="search">
                        ...
                        <table id="example3" class="display">
                            ...
                            @foreach($categories as $item)
                                <tr>
                                    ...
                                    <td>{{ $item->enabled }}</td>
                                    <td>@if ($item->parent) <a href="{{ $item->parent->backendLink() }}" ><span class="badge badge-sm light badge-primary">{{ $item->parent->title }}</span> </a> @else - @endif</td>
                                    <td class="text-center">
                                        <a href="{{ url('/admin/categories/' . $item->id) }}" title="Detalles"><button class="shadow btn btn-info btn-xs sharp me-1"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                                        <a href="{{ url('/admin/categories/' . $item->id . '/edit') }}" title="Editar"><button class="shadow btn btn-primary btn-xs sharp me-1"><i class="fa fa-pencil" aria-hidden="true"></i></button></a>
