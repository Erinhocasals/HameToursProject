        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ url('/admin/filter-categories/create') }}" class="btn btn-primary btn-sm" title="Adicionar nueva Categoría">
                        <i class="fa fa-plus" aria-hidden="true"></i> Adicionar nueva
                    </a>
                </div>
                <div class="card-body">
                    {{-- <form method="GET" action="{{ url('/admin/filter-categories') }}" accept-charset="UTF-8" class="float-right my-2 form-inline my-lg-0" role="search">
                    ...
                    <div class="table-responsive">
                            ...
                            @foreach($filtercategories as $item)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <a href="{{ url('/admin/filter-categories/' . $item->id) }}" title="Detalles"><button class="shadow btn btn-info btn-xs sharp me-1"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                                        <a href="{{ url('/admin/filter-categories/' . $item->id . '/edit') }}" title="Editar"><button class="shadow btn btn-primary btn-xs sharp me-1"><i class="fa fa-pencil" aria-hidden="true"></i></button></a>

                                        <form method="POST" action="{{ url('/admin/filter-categories' . '/' . $item->id) }}" accept-charset="UTF-8" style="display:inline">
                                            {{ method_field('DELETE') }}
                                            {{ csrf_field() }}
                        ...
                        <div class="pagination-wrapper"> {!! $filtercategories->appends(['search' => Request::get('search')])->render() !!} </div>
                    </div>
