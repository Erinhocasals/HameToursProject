...
@section('content')
    <x-jet-authentication-card>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mt-4">
                <x-jet-label for="phone" value="{{ __('validation.attributes.phone') }}" />
                <x-jet-input id="phone" class="block w-full mt-1" type="text" name="phone" :value="old('phone')" required />
            </div>
            ...
            <div class="flex items-center justify-end mt-4">
                <a class="text-sm text-gray-600 underline hover:text-gray-900" href="@if (app('request')->input('referral')) {{ route('login', ['referral' => app('request')->input('referral')]) }} @else {{ route('login') }} @endif">
